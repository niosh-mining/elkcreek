...
ml = np.array([-0.39, -0.48])
mw = np.array([0.40, 0.45])
...
class TestApparentVolume:
    ...
    def test_apparent_volume_from_apparent_stress(self):
        ...
        np_assert(
            apparent_volume(moments, apparent_stress=apparent_stresses),
            apparent_volumes,
            rtol=0.05,
        )
    ...
    def test_apparent_volume_from_energy_shear(self):
        ...
        np_assert(
            apparent_volume(moment=moments, energy=energies, shear_modulus=shear_mod),
            apparent_volumes,
            rtol=0.05,
        )
    ...
    def test_from_apparent_stress_unnecessary_kwargs(self):
        ...
        with pytest.raises(ValueError, match="Unused keyword arguments"):
            apparent_volume(
                moment=moments,
                apparent_stress=apparent_stresses,
                energies=energies,
                shear_mod=shear_mod,
            )
    ...
    def test_single_value_instead_of_iterable(self):
        ...
        np_assert(
            apparent_volume(moments[0], apparent_stress=apparent_stresses[0]),
            apparent_volumes[0],
            rtol=0.05,
        )
...
class TestMagnitudes:
    def test_moment_mag(self):
        """Make sure moment magnitudes are correct"""
        np_assert(moment_magnitude(moments), mw, atol=0.01)

    def test_local_mag(self):
        """Make sure local magnitudes are correct"""
        np_assert(local_magnitude(moments, energies), ml, atol=0.01)
