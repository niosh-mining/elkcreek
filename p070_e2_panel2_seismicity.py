...
def plot_map(events, bursts, reference_positions, ax, cbar_ax):
    """Plot map of events on Panel 2 surrounding Event 2"""
    # Grab the face positions for the two times of interest
    faces = pd.DataFrame(
        [
            reference_positions.loc[
                reference_positions.date == pd.Timestamp(local.p2_face_start.date())
            ].iloc[0],
            reference_positions.loc[
                reference_positions.date == pd.Timestamp(local.p2_face_end.date())
            ].iloc[0],
        ]
    )

    plot_over_advance_window(
        faces.iloc[0],
        faces.iloc[1],
        events,
        bursts,
        ...
        cbar_ax=cbar_ax,
        extents=local.map_extents_event2,
    )
...
def plot_daily_event_count(events, bursts, reference_positions, ax):
    """Plot daily events surrounding Event 2"""
    e2 = bursts.iloc[1].time
    anomalous_date = get_date_from_face_position(
        local.panel2_anomalous_event2, reference_positions
    )
    anomalous_days = (anomalous_date - e2).days

    sub = events.loc[
        (events.time >= local.p2_time_series_start)
        & (events.time < local.p2_time_series_end)
    ]
    sub["event_count"] = 1
...
def main():
    ...
    daily_face_positions = compile_daily_face_positions(
        "2011-01-01", "2011-05-01", longwall_df
    )
